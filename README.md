...
- [Keeping the Contents sections in sync](#keeping-the-contents-sections-in-sync)
...
## Keeping the Contents sections in sync

Every markdown page (and this README) opens with a `## Contents` list of its `##` and
`###` headings. It's generated by [`add_toc.py`](add_toc.py) — don't hand-edit it. After
adding, removing, or renaming a heading, run `python add_toc.py` (no arguments = all
files) to refresh it. The script only ever touches the Contents section and is
idempotent, so re-running is safe. Keep `#` for the title alone.
